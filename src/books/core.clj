...
(defroutes app-routes
  (GET "/bookshelf" request
   ...
   (POST "/bookshelf" request
    ...
    (GET "/account/reset-password" [] (friend/authorize #{::user}(views/pwd-reset)))
    (POST "/account/reset-password" request (friend/authorize #{::user}(helpers/password-reset-check (get-in request [:session :cemerick.friend/identity :current]) (get-in request [:params :old-password])(get-in request [:params :new-password])(get-in request [:params :new-password-reenter]))))
  (route/resources "/")
